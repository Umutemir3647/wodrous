    ...
    async execute(interaction) {
        ...
        if (interaction.channelId !== config.premiumChannel) {
            const wrongChannelEmbed = new MessageEmbed()
                ...
                .setDescription(`Bu kanalda premium komudunu kullanamazsın <#${config.premiumChannel}> kanalında tekrar dene!`)
        ...
        if (generated.has(member.id)) {
            const cooldownEmbed = new MessageEmbed()
                .setColor(config.color.red)
                .setTitle('Cooldown!')
                .setDescription(`Lütfen **${config.premiumCooldown}** saniye sonra tekrar deneyin!`)
                .setFooter(interaction.user.tag, interaction.user.displayAvatarURL({ dynamic: true, size: 64 }))
                .setTimestamp();
        ...
        fs.readFile(filePath, 'utf-8', (error, data) => {
            ...
            fs.writeFile(filePath, updatedData, (writeError) => {
                ...
                const embedMessage = new MessageEmbed()
                    ...
                    .setTitle('Premium hesap üretildi')
                    .setFooter(interaction.user.tag, interaction.user.displayAvatarURL({ dynamic: true, size: 64 }))
                ...
                interaction.reply({
                    content: `**DM'ni kontrol et ${member}!** __Eğer mesaj almadıysan DM'lerini aç!__`,
                });
